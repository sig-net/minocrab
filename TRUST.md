...
| `minocrab-ledger/src/reads.rs` | 775 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
| `minocrab-ledger/src/kernel.rs` | 344 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
| `minocrab-ledger/src/calls.rs` | 312 | differential_baseline (call-compatibility with compactc's artifacts); every contract differential; nested_differential + nested_typed (nested paths); entry_point (315 contracts). Ops no fixture reaches (VERIFICATION.md §5 'unported constructs'): **READING** against midnight-ledger.ss vm-code | an Impact op encoded so the ledger applies a different state change than the circuit claims |
...
25644 lines in the seven crates; 10732 of them in files whose warrant is READING in whole or in part (the rows in bold).
<!-- GENERATED END -->
