...
| `ContractAddress<Public>` | passing a bare `B32` where `kernel.self()` was meant — the `_with` twins take the typed address, so a raw pair of limbs no longer type-checks there. NOT a proof of self: `ContractAddress` is Compact's value type (recipients, call targets, `Either` arms) and is built from any `B32` in a dozen places; a `SelfAddress` newtype that only `kernel::self_address` produces ([notes/api-safety-survey.org §3 item 5](notes/api-safety-survey.org)) would be the real closure and is not built | zero rows, dump unchanged ([notes/api-safety-survey.org §A4](notes/api-safety-survey.org)) |
