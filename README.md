...
- Bounded integers that are not a power of two. Compact's `Uint<0..n>` takes any bound; our leaf is `Uint<BITS>` ([v3.rs](crates/minocrab-std/src/v3.rs)). Handled on the interop side ([abi.rs](crates/minocrab/src/v3/abi.rs)), not declarable. Non-power-of-two `enum`s fall in this class.
